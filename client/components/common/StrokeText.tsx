...
const StrokeText = ({
  ...
  const isHighlighted = (index: number) => highlightRanges.some(([start, end]) => index >= start && index < end);

  const fontPx = Number(fontSize) || 128;
  const dash = Math.max(fontPx * 7, 200);
  ...
  const fontStyle = useMemo(
    ...
    [fontPx, fontWeight, letterSpacing]
  );
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, fontPx * 0.1);
  ...
  }, [characters, fontPx, fontWeight, letterSpacing, strokeWidth]);
  ...
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fontPx} 600 ${fontPx * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={className}
      style={{ display: "block", width: "100%", lineHeight: 0, ...(trigger === "hover" ? { cursor: "pointer" } : {}), ...style }}
      role="img"
      ...
      >
        ...
        >
          {characters.map((char, index) => (
            <tspan data-stroke-char key={`s-${index}`} stroke={isHighlighted(index) ? highlightColor : undefined}>{char}</tspan>
          ))}
        </text>
        ...
        >
          {characters.map((char, index) => (
            <tspan data-fill-char key={`f-${index}`} fill={isHighlighted(index) ? highlightColor : undefined}>{char}</tspan>
          ))}
        </text>
      </svg>
    </span>
