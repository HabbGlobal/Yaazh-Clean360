...
// Component from React Bits (StrokeText) — adapted to inline styles (no Tailwind)
...
export default function StrokeText({
  ...
  const fs = Number(fontSize) || 128;
  const fw = Number(fontWeight) || 800;
  const ls = Number(letterSpacing) || 0;

  const dash = Math.max(fs * 7, 200);

  const fontStyle = useMemo<CSSProperties>(
    () => ({ fontSize: `${fs}px`, fontWeight: fw, letterSpacing: `${ls}px` }),
    [fs, fw, ls]
  );
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, fs * 0.1);
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  }, [characters, fs, fw, ls, strokeWidth]);
  ...
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fs} 600 ${fs * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`${trigger === "hover" ? "cursor-pointer" : ""} ${className}`.trim()}
      style={{ display: "inline-block", width: "auto", lineHeight: 0, ...style }}
      role="img"
      aria-label={String(text ?? "")}
    >
      <svg
        className="stroke-text-svg"
        style={{ display: "block", width: "auto", height: `${Math.round(fs * 1.3)}px` }}
        viewBox={viewBox}
